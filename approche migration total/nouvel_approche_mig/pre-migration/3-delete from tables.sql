    ...
    delete from  GENADRESS where adr_id not in (select adr_id from genparty where par_id in (select org_id from genorganization) union select adr_id from genagent union select 0 from dual);  
    commit;
