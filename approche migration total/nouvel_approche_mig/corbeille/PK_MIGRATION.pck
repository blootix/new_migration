  ...
  IS
    ...
    is
     select lpad(trim(b.district),2,'0') district, lpad(trim(b.police),5,'0') police, lpad(trim(b.tourne),3,'0') tourne, lpad(trim(b.ordre),3,'0') ordre, b.adresse, date_creation,
            lpad(trim(categorie_actuel),2,'0') categorie_actuel, upper(trim(client_actuel)) client_actuel, lpad(trim(b.code_marque),3,'0') code_marque, lpad(trim(b.compteur_actuel),11, '0') compteur_actuel, 
            b.code_postal,trim(b.usage) usage, b.type_branchement, b.aspect_branchement, b.marche, trim(b.etat_branchement) etat_branchement,
            trim(t.banque) banque, trim(t.agence) agence, trim(t.num_compte) num_compte, trim(t.cle_rib) cle_rib
