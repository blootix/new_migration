...
prompt *******************************************************************
update pivot.MIWTRELEVE_HISTOCPT set  mrel_cadran1 ='EAU' where mrel_cadran1 is null ;
update pivot.MIWTRELEVE_HISTOCPT set  VOC_COMM1 ='0' where VOC_COMM1 is null ;
update pivot.MIWTRELEVE_HISTOCPT set  VOC_READMETH ='Inconn' where VOC_READMETH is null ;
update pivot.MIWTRELEVE_HISTOCPT set  MREL_SUBREAD =0 where MREL_SUBREAD is null ;

ANALYZE TABLE pivot.MIWTRELEVE_HISTOCPT COMPUTE STATISTICS;
UPDATE pivot.MIWTRELEVE_HISTOCPT m
SET    equ_id =
           (
               SELECT equ_id 
               FROM   pivot.miwtcompteur c 
...
WHERE spt_id is null;
		...
		update pivot.MIWTRELEVE_HISTOCPT  
...
PROMPT *******************************************************
PROMPT *** MIGRATION  / Création pivot.MIWTRELEVE_HISTOCPT ****************
PROMPT *******************************************************
SET serveroutput on;
Declare 
Cursor Releve  is select
					...
					mrel_ano_f1        ,
					mrel_ano_n1
			from    pivot.MIWTRELEVE_HISTOCPT 
...
--select nvl(max(mrd_id),0)+1 into Rel_Id      from tecmtrread;
  ...
  New_Rel:= null;
--------------------------------------------------------Voc ne pas traiter 
--select distinct t.mrel_ano_n1 into v_VOW_COMM1 from pivot.MIWTRELEVE_HISTOCPT t
...
--alter table pivot.MIWTRELEVE_HISTOCPT add MEU_ID number;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID     = null WHERE  MEU_ID    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID2    = null WHERE  MEU_ID2    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID3    = null WHERE  MEU_ID3    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID4    = null WHERE  MEU_ID4    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID5    = null WHERE  MEU_ID5    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID6    = null WHERE  MEU_ID6    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID7    = null WHERE  MEU_ID7    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID8    = null WHERE  MEU_ID8    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID9    = null WHERE  MEU_ID9    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MEU_ID10   = null WHERE  MEU_ID10    is not null;

--alter table pivot.MIWTRELEVE_HISTOCPT add MME_ID number;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID     = null WHERE  MME_ID    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID2    = null WHERE  MME_ID2    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID3    = null WHERE  MME_ID3    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID4    = null WHERE  MME_ID4    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID5    = null WHERE  MME_ID5    is not null;
UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID6    = null WHERE  MME_ID6    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID7    = null WHERE  MME_ID7    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID8    = null WHERE  MME_ID8    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID9    = null WHERE  MME_ID9    is not null;
--UPDATE pivot.MIWTRELEVE_HISTOCPT SET MME_ID10   = null WHERE  MME_ID10    is not null;
...
v_pas  integer;
BEGIN
select min(mrd_id) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(mrd_id) into v_max from pivot.MIWTRELEVE_HISTOCPT;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
		update  /*+rule*/ pivot.MIWTRELEVE_HISTOCPT r
...
v_pas  integer;
BEGIN
select min(MME_ID) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
 ...
 (
	SELECT  /*+RULE*/
									...
									0				,
									MREL_DEDUITE
	FROM     pivot.MIWTRELEVE_HISTOCPT R
...
v_pas  integer;
BEGIN
select min(MME_ID2) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID2) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
...
v_pas  integer;
BEGIN
select min(MME_ID3) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID3) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
...
v_pas  integer;
BEGIN
select min(MME_ID4) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID4) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
...
v_pas  integer;
BEGIN
select min(MME_ID5) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID5) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
...
v_pas  integer;
BEGIN
select min(MME_ID6) into v_min from pivot.MIWTRELEVE_HISTOCPT ;
select max(MME_ID6) into v_max from pivot.MIWTRELEVE_HISTOCPT ;
v_pas := round((v_max-v_min+1)/10) + 1;
v_depart := v_min;
while v_depart <= v_max
loop
