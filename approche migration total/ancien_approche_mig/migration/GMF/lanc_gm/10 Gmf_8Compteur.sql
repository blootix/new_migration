...
update  pivot.miwtcompteur set MCOM_REFVOCMARQUE = '-' where MCOM_REFVOCMARQUE is null ;
update  pivot.miwtcompteur set VOC_MODEL = '-' where VOC_MODEL is null ;
update  pivot.miwtcompteur set VOC_DIAM ='-'  where VOC_DIAM is null ;
update  pivot.miwtcompteur set VOC_MTRELECTP ='-'  where VOC_MTRELECTP is null ;
update  pivot.miwtcompteur set MCOM_REFVOCCLASSE ='-'  where MCOM_REFVOCCLASSE  is null ;
update  pivot.miwtcompteur set mcom_annee =to_char(sysdate ,'yyyy')  where mcom_annee  is null ;
update  pivot.miwtcompteur set MCOM_RADI_CONS = '-' where MCOM_RADI_CONS is null ;
update  pivot.miwtcompteur set MCOM_RADI_MODELE = '-' where MCOM_RADI_MODELE is null ;
update  pivot.miwtcompteur set mcom_radi_annee = 2014 where mcom_radi_annee is null ;
update  pivot.miwtcompteur set VOC_OWNER = 'ELD' where VOC_OWNER is null ;
UPDATE pivot.miwtcompteur SET mcom_annee=2000 where length(mcom_annee)<4;
PROMPT *******************************************************
...
--select nvl(max(eqy_id),0) into tecequtype_Id     from tecequtype;
	...
	For Equi in Compteur loop
       ...
       -- select f.fld_id into New_Tecmtrcfg.FLD_ID  from tecfluid f where fld_code ='PDC EAU' ;  --fld_code = 'EAU' ;
		...
		PK_Util_Vocabulaire.Genere_voc( New_tecmtrwater.VOW_DIAM ,'VOW_DIAM', New_tecmtrwater.VOC_DIAM) ;--ocabulaire Fabricant[MANUFACT]
		PK_Util_Vocabulaire.Genere_voc( New_tecmtrwater.VOW_CLASS ,'VOW_CLASS', New_tecmtrwater.MCOM_REFVOCCLASSE) ;--ocabulaire Fabricant[MANUFACT]
        PK_Util_Vocabulaire.Genere_voc( New_tecmtrwater.VOW_QN ,'VOW_QN', New_tecmtrwater.VOC_QN) ;--ocabulaire Fabricant[MANUFACT]	
                       begin		
	...
	IF Equi.mcom_radi_refe Is not null then 
       ...
       -- select f.fld_id into New_Tecmtrcfg.FLD_ID  from tecfluid f where fld_code ='PDC EAU' ;  --fld_code = 'EAU' ;
		...
		Genere_Compteur (New_Equi_Radio ,Equi_Id  ,ref_Equi_Radio ) ;
					...
					Update  pivot.miwtcompteur 
					set equ_id_radio =ref_Equi_Radio  
					where MCOM_REF = Equi.MCOM_REF ;
		
    End if;
	Commit;
	Update  pivot.miwtcompteur set equ_id =ref_Equi where MCOM_REF = Equi.MCOM_REF ;
...
/
		...
		update pivot.miwthistocpt  
		set MHPC_AGESOURCE ='INCONNUE' 
		where  MHPC_AGESOURCE is null;

	UPDATE pivot.miwthistocpt m
		SET    AGE_ID =
				   (
					   SELECT AGE_ID 
					   FROM   pivot.miwagent c 
...
Declare 
 ...
 w_voc_dep_id Number ;
Begin
For voc in  (select VOC_REASPOSE ,MHPC_REFCOM,MHPC_REFPDL,VOC_REASDEPOSE from  pivot.miwthistocpt )
loop 
...
prompt **********************************************************************
ANALYZE TABLE pivot.miwthistocpt COMPUTE STATISTICS;
ANALYZE TABLE  pivot.miwtcompteur COMPUTE STATISTICS;
ANALYZE TABLE  pivot.miwtpdl COMPUTE STATISTICS;
