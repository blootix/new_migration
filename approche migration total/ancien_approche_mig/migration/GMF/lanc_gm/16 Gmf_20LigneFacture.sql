--update miwtreleve s set s.mrel_fact1=(select mrel_fact1 from pivot.miwtreleve t where t.mrel_ref=s.mrel_ref)where s.mrel_fact1 is null;
--commit;
spo Gmf_11Fac.lst;
SELECT to_char(sysdate,'DD/MM/YY HH24:MI') heure FROM dual;
ANALYZE TABLE pivot.miwtfactureentete COMPUTE STATISTICS;
ANALYZE TABLE pivot.miwthistopayeur COMPUTE STATISTICS;
ANALYZE TABLE genorganization COMPUTE STATISTICS;
ANALYZE TABLE pivot.miwabn COMPUTE STATISTICS;
...
Update pivot.miwtfactureentete set MFAE_REF_CODNIV_RELANCE='INCONNUE' where  MFAE_REF_CODNIV_RELANCE Is null ;
Update pivot.miwtfactureentete set MFAE_RIB_ETAT = 4 where  MFAE_RIB_ETAT Is null ;
SELECT to_char(sysdate, 'DD/MM/YY HH24:MI') heure FROM dual;
PROMPT *******************************************************
PROMPT *** MIGRATION  / Création pivot.miwtfactureentete***********
PROMPT *******************************************************
SET serveroutput on;
Declare
Cursor Facture  is select
    ...
    MFAE_NUMEROROLE,
    MFAE_PREL
from    pivot.miwtfactureentete fac ,
		pivot.miwthistopayeur pay ,
		pivot.miwabn abn ,
		genorganization g,
		agrserviceagr ss
		
where   fac.MFAE_SOURCE   = ABN_SOURCE
and     fac.MFAE_REFABN=abn.abn_refpdl 
...
For Fac in Facture loop
	...
	select seq_genaccount.nextval into genaccount_id from dual;
		...
		select count(*) 
		into v 
		from pivot.miwtfactureentete e 
...
--
    ...
    --New_Det.DEB_CREDT        :=
		...
		exception when others then
			insert into pivot.prob_migration_save 
				            (nom_table,
						     val_ref,
							 sql_err
							)
    ...
    New_Genbill.RUN_ID            :=  genrun_ref  ;
		...
		exception when others then 
								insert into pivot.prob_migration_save (nom_table,val_ref,sql_err)
							   values('genbil',
								New_Genbill.BIL_ID       ||'--'||
								New_Genbill.BIL_CODE	 ||'--'||
								New_Genbill.BIL_CALCDT   ||'--'||
    ...
    Update pivot.miwtfactureentete 
	set BIL_ID = Agrbill_Id ,
	DEB_ID =Debt_Id 
	where mfae_source = Fac.mfae_source 
	and MFAE_REF = Fac.MFAE_REF ;
...
-----------------
    v := 0;
    for c in (select * from pivot.miwtfactureligne e  where fac.mfae_ref=e.mfal_reffae)
    loop
         v := v+1;
                ...
                begin
						...
						from genitem s 
						where s.ITE_CODE = c.MFAL_REFART;
...
--------Lien entre les factures, avoirs, refacturation
	for fac_ in (select * from pivot.miwtfactureentete where MFAE_REF_ORIGINE is not null) 
	loop
