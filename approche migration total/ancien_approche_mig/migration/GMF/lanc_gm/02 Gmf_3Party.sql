...
begin
  Update pivot.MIWTPERSONNE set VOC_TYPSAG =1  where VOC_TYPSAG is null ;
  Update pivot.MIWTPERSONNE set VOC_PARTYTP =1 where VOC_PARTYTP is null ;
  update pivot.miwtpersonne 
  ...
  For Per in Client loop
    if (pre.mper_refe<>(select PAR_REFE from genparty) )then
	 Update pivot.miwtpersonne set PAR_ID= v_ref_per ,ADR_ID =Per.ADR_ID ,
