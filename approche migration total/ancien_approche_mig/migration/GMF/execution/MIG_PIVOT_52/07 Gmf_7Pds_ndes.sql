...
PROMPT ********************************************************/
--update  mig_pivot_52.miwabn a set CTT_ID = (select c.ctt_id from  mig_pivot_52.miwctrtype c where c.ctrtype_ref = a.ABN_CTRTYPE_REF ) where CTT_ID is null ;
...
is 
  select  ABN_source        ,
      ...
      --  drois de coupure [CUTAGREE]
          ...
          a.migabn_cr		,
          p.adr_id
   from    mig_pivot_52.miwabn A,
           mig_pivot_52.MIWTPERSONNE p 
...
is 
  select MHPA_SOURCE        ,
         ...
         P.PAR_ID           ,
         VOC_SETTLEMODE
  from    mig_pivot_52.MIWTHISTOPAYEUR,
          mig_pivot_52.MIWTPERSONNE  P 
...
BEGIN
  update  mig_pivot_52.MIWABN set PRE_ID =(select PRE_ID from  mig_pivot_52.miwtpdl where ABN_REFPDL = PDL_REF ) where PRE_ID  is null ;  
  update  mig_pivot_52.MIWABN set SPT_ID =(select SPT_ID from  mig_pivot_52.miwtpdl where ABN_REFPDL = PDL_REF ) where SPT_ID  is null ;
  ---- Affecte delete le payeur que ne pas le dernier
  update   mig_pivot_52.MIWABN set ABN_REFPER_A =(select MHPA_REF_PAYEUR from  mig_pivot_52.MIWTHISTOPAYEUR where MHPA_REF_ABNT = ABN_REF ) where ABN_REFPER_A  is null ; 
  update   mig_pivot_52.MIWTHISTOPAYEUR set PAR_ID  =(select PAR_ID from  mig_pivot_52.miwtpersonne where MHPA_REF_PAYEUR = MPER_REF ) where MHPA_REF_PAYEUR  is not null ; 
  update   mig_pivot_52.MIWTHISTOPAYEUR set ADR_ID  =(select ADR_ID from  mig_pivot_52.miwtpersonne where MHPA_REF_PAYEUR = MPER_REF ) where MHPA_REF_PAYEUR  is not null ;
    ...
    /*
        ...
        insert into GENPARTYPARTY
               ...
               values(
				...
				Update  mig_pivot_52.miwthistopayeur  
				set pay_id =  ref_pay 
