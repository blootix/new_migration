...
update  mig_pivot_05.miwtpdl l set l.VOC_CONSTATUS='I' where upper(trim(l.VOC_CONSTATUS)) not in('B','M','I');
update mig_pivot_05.miwtpdl l set l.VOC_CONNAT=0 where  to_char(trim(l.VOC_CONNAT)) not in('0','1','2','3');

declare
cursor c1
is 
  select t.spt_id,decode(m.VOC_CONSTATUS,'B','B','M','M','I','I','I')VOC_CONSTATUS,decode(m.VOC_CONNAT,0,0,1,1,2,2,3,3,0)VOC_CONNAT 
  from tecservicepoint t,  mig_pivot_05.miwtpdl m where m.spt_id=t.spt_id;
