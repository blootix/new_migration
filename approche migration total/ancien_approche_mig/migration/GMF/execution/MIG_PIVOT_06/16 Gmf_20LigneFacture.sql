--update miwtreleve s set s.mrel_fact1=(select mrel_fact1 from  MIG_PIVOT_06.miwtreleve t where t.mrel_ref=s.mrel_ref)where s.mrel_fact1 is null;
--commit;
spo Gmf_11Fac.lst;
SELECT to_char(sysdate, 'DD/MM/YY HH24:MI') heure FROM dual;
ANALYZE TABLE  MIG_PIVOT_06.miwtfactureentete COMPUTE STATISTICS;
ANALYZE TABLE  MIG_PIVOT_06.miwthistopayeur COMPUTE STATISTICS;
ANALYZE TABLE genorganization COMPUTE STATISTICS;
ANALYZE TABLE  MIG_PIVOT_06.miwabn COMPUTE STATISTICS;
...
Update  MIG_PIVOT_06.miwtfactureentete set MFAE_REF_CODNIV_RELANCE = 'INCONNUE' where  MFAE_REF_CODNIV_RELANCE Is null ;
update  mig_pivot_06.MIWTFACTUREENTETE r set r.MFAE_DLPAI=r.MFAE_DEDI+15 where r.MFAE_DLPAI is null ;
Update  MIG_PIVOT_06.miwtfactureentete set MFAE_RIB_ETAT = 4 where  MFAE_RIB_ETAT Is null ;

PROMPT *******************************************************
PROMPT *** MIGRATION  / Création  MIG_PIVOT_06.miwtfactureentete***********
PROMPT *******************************************************
SET serveroutput on;
Declare
Cursor Facture  is select
    ...
    MFAE_NUMEROROLE,
    MFAE_PREL
from     MIG_PIVOT_06.miwtfactureentete fac ,
		 MIG_PIVOT_06.miwthistopayeur pay ,
		 MIG_PIVOT_06.miwabn abn ,
		genorganization g,
		agrserviceagr ss
...
select seq_genrun.nextval     into genrun_id     from dual;
		...
		select count(*) into v 
		from  MIG_PIVOT_06.miwtfactureentete e 
...
--
    ...
    Update  MIG_PIVOT_06.miwtfactureentete 
...
-----------------
    v := 0;
    for c in (select * from  MIG_PIVOT_06.miwtfactureligne e  where fac.mfae_ref=e.mfal_reffae)
         loop
         v := v+1;
...
for fac_ in (select * from  MIG_PIVOT_06.miwtfactureentete where MFAE_REF_ORIGINE is not null) loop
...
--commit;
--			insert into TECREADITEBILL(select l.bil_id,l.ite_id,t.mrd_id,b.bil_calcdt,null,null,0 from miwtreleve t, MIG_PIVOT_06.miwtfactureentete f,genbill b,genbilline l
