...
update   mig_pivot_45.miwtcompteur set MCOM_REFVOCMARQUE = '-' where MCOM_REFVOCMARQUE is null ;
update   mig_pivot_45.miwtcompteur set VOC_MODEL = '-' where VOC_MODEL is null ;
update   mig_pivot_45.miwtcompteur set VOC_DIAM ='-'  where VOC_DIAM is null ;
update   mig_pivot_45.miwtcompteur set VOC_MTRELECTP ='-'  where VOC_MTRELECTP is null ;
update   mig_pivot_45.miwtcompteur set MCOM_REFVOCCLASSE ='-'  where MCOM_REFVOCCLASSE  is null ;
update   mig_pivot_45.miwtcompteur set mcom_annee =to_char(sysdate ,'yyyy')  where mcom_annee  is null ;
update   mig_pivot_45.miwtcompteur set MCOM_RADI_CONS = '-' where MCOM_RADI_CONS is null ;
update   mig_pivot_45.miwtcompteur set MCOM_RADI_MODELE = '-' where MCOM_RADI_MODELE is null ;
update   mig_pivot_45.miwtcompteur set mcom_radi_annee = 2014 where mcom_radi_annee is null ;
update   mig_pivot_45.miwtcompteur set VOC_OWNER = 'ELD' where VOC_OWNER is null ;
UPDATE  mig_pivot_45.miwtcompteur SET mcom_annee=2000 where length(mcom_annee)<4;
...
Cursor Compteur  is select
        ...
        MCOM_NBMEU         ,
		mtc_id
        from     mig_pivot_45.miwtcompteur 
       ...
       -- select f.fld_id into New_Tecmtrcfg.FLD_ID  from tecfluid f where fld_code ='PDC EAU' ;  --fld_code = 'EAU' ;
		...
		--Genere_tecmtrcfg (New_Tecmtrcfg,Tecmtrcfg_Id ,ref_Tecmtrcfg ) ;
		         begin
				   ...
				   err_mess := sqlerrm ;
				   --insert into  mig_pivot_45.MIW_LOG(MIW_LOG_TAB,MIW_LOG_ERRUR)
		...
		Update   mig_pivot_45.miwtcompteur 
		set equ_id =ref_Equi  
...
/
		...
		update  mig_pivot_45.miwthistocpt  
		set MHPC_AGESOURCE ='INCONNUE' 
		where  MHPC_AGESOURCE is null;

	UPDATE  mig_pivot_45.miwthistocpt m
		SET    AGE_ID =
				   (
					   SELECT AGE_ID 
					   FROM    mig_pivot_45.miwagent c 
...
Declare 
 ...
 w_voc_dep_id Number ;
Begin
For voc in  (select VOC_REASPOSE ,MHPC_REFCOM,MHPC_REFPDL,VOC_REASDEPOSE from   mig_pivot_45.miwthistocpt )
loop 
...
prompt **********************************************************************
ANALYZE TABLE  mig_pivot_45.miwthistocpt COMPUTE STATISTICS;
ANALYZE TABLE   mig_pivot_45.miwtcompteur COMPUTE STATISTICS;
ANALYZE TABLE   mig_pivot_45.miwtpdl COMPUTE STATISTICS;
 ...
 (
  ...
  (
    ...
    from     mig_pivot_45.miwtcompteur  c  ,
             mig_pivot_45.miwthistocpt h   ,
             mig_pivot_45.miwtpdl  p
    where 1 = 1
 ...
 (
  ...
  (
    select v_debut_id + rownum, 
           ...
           MCOM_RADI_POSE    
    from     mig_pivot_45.miwtcompteur   Equi
    where 1 = 1
	and   EQU_ID        is not null
