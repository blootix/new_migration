--update miwtreleve s set s.mrel_fact1=(select mrel_fact1 from  mig_pivot_42.miwtreleve t where t.mrel_ref=s.mrel_ref)where s.mrel_fact1 is null;
--commit;
spo Gmf_11Fac.lst;
SELECT to_char(sysdate, 'DD/MM/YY HH24:MI') heure FROM dual;
ANALYZE TABLE  mig_pivot_42.miwtfactureentete COMPUTE STATISTICS;
ANALYZE TABLE  mig_pivot_42.miwthistopayeur COMPUTE STATISTICS;
ANALYZE TABLE genorganization COMPUTE STATISTICS;
ANALYZE TABLE  mig_pivot_42.miwabn COMPUTE STATISTICS;
...
Update  mig_pivot_42.miwtfactureentete set MFAE_REF_CODNIV_RELANCE = 'INCONNUE' where  MFAE_REF_CODNIV_RELANCE Is null ;
update  mig_pivot_42.MIWTFACTUREENTETE r set r.MFAE_DLPAI=r.MFAE_DEDI+15 where r.MFAE_DLPAI is null ;
Update  mig_pivot_42.miwtfactureentete set MFAE_RIB_ETAT = 4 where  MFAE_RIB_ETAT Is null ;

PROMPT *******************************************************
PROMPT *** MIGRATION  / Création  mig_pivot_42.miwtfactureentete***********
PROMPT *******************************************************
SET serveroutput on;
Declare
Cursor Facture  is select
    ...
    MFAE_NUMEROROLE,
    MFAE_PREL
from     mig_pivot_42.miwtfactureentete fac ,
		 mig_pivot_42.miwthistopayeur pay ,
		 mig_pivot_42.miwabn abn ,
		genorganization g,
		agrserviceagr ss
...
select seq_genrun.nextval     into genrun_id     from dual;
		...
		select count(*) into v 
		from  mig_pivot_42.miwtfactureentete e 
...
--
    ...
    Update  mig_pivot_42.miwtfactureentete 
...
-----------------
    v := 0;
    for c in (select * from  mig_pivot_42.miwtfactureligne e  where fac.mfae_ref=e.mfal_reffae)
         loop
         v := v+1;
...
for fac_ in (select * from  mig_pivot_42.miwtfactureentete where MFAE_REF_ORIGINE is not null) loop
...
--commit;
--			insert into TECREADITEBILL(select l.bil_id,l.ite_id,t.mrd_id,b.bil_calcdt,null,null,0 from miwtreleve t, mig_pivot_42.miwtfactureentete f,genbill b,genbilline l
