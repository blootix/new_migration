			...
			select seq_tecmtrmeasure.nextval into v_MME_ID from dual;
					INSERT INTO tecmtrmeasure
										...
										(
											...
											v_MEU_ID      	, 
											2    	        , 
0											to_number(replace(releve_.releve2,'.',null)),
			...
			select seq_tecmtrmeasure.nextval into v_MME_ID from dual;
					INSERT INTO tecmtrmeasure
										...
										(
											...
											v_MEU_ID      	, 
											3    	        , 
0											to_number(replace(releve_.releve3,'.',null)),
			...
			select seq_tecmtrmeasure.nextval into v_MME_ID from dual;
					INSERT INTO tecmtrmeasure
										...
										(
											...
											v_MEU_ID      	, 
											4    	        , 
0											to_number(replace(releve_.releve4,'.',null)),
			...
			select seq_tecmtrmeasure.nextval into v_MME_ID from dual;
					INSERT INTO tecmtrmeasure
										...
										(
											...
											v_MEU_ID      	, 
											5    	        , 
0											to_number(replace(releve_.releve5,'.',null)),
        ...
        WHEN OTHERS THEN
            ...
            exception when others then
             ...
             if substr(releve_.annee,3,2) !=to_char(to_date(substr(releve_.date_releve,1,10),'dd/mm/yyyy hh24:mi:ss'),'YY') and v_trim in (1, 2, 3) then
               ...
               if v_NSIXIEME = 1 THEN
                    IF v_trim = 1 THEN
                      v_mrd_dt := to_date('08/0' || to_char(v_NTIERS) || '/' ||
                                              releve_.annee,
                                              'dd/mm/yyyy');
                    ELSIF v_trim = 2 THEN
                      v_mrd_dt := to_date('08/0' || to_char(3 + v_NTIERS) || '/' ||
                                              releve_.annee,
                                              'dd/mm/yyyy');
                    ELSIF v_trim = 3 THEN
                      v_mrd_dt := to_date('08/0' || to_char(6 + v_NTIERS) || '/' ||
                                              releve_.annee,
                                              'dd/mm/yyyy');
                    ELSIF v_trim = 4 THEN
                      v_mrd_dt := to_date('08/' || to_char(9 + v_NTIERS) || '/' ||
                                              releve_.annee,
                                              'dd/mm/yyyy');
                    end if;
               else
