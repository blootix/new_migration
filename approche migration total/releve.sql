declare 
	...
	CURSOR releve_gc select distinct a.nindex,
                    ...
                    and   lpad(trim(a.dist),2,'0')=lpad(trim(c.district),2,'0');
...
---------------------------------------------------------------------------------------------		
	    FOR releve_ in releve LOOP
		    FOR x in branch_(lpad(trim(releve_.district),2,'0'),lpad(trim(releve_.tourne),3,'0'),lpad(trim(releve_.ordre),3,'0'))LOOP			   
				V_VOW_READREASON  :=null;
				V_RL_LECIMP       :=null;
				V_mrd_comment     :=null;
				V_DTE_RL          :=null;
...
---------------------------recuperation date releve------------------------
				...
				BEGIN
					...
					into V_mrd_dt 
					from ROLE_TRIM t
					where lpad(TRIM(t.distr),2,'0')=lpad(trim(releve_.district),2,'0')
